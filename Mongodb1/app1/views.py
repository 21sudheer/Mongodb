...
def get_sum(request):
    trip_id ='63d4228e01afc1d6b6813e38'
    sensor_name="GPS"
    sensor_sum=db.vesseltripdata.aggregate([{"$match":{"tripId":trip_id,'sensorName':sensor_name}},{"$addFields": {"arraySize":{"$size":"$dataPoints"}}},{"$group": {"_id":"$sensorName","index0_sum":{"$sum":{"$arrayElemAt":["$dataPoints",0]}},"index1_sum":{"$sum":{"$arrayElemAt":["$dataPoints",1]}},"index2_sum":{"$sum":{"$arrayElemAt":["$dataPoints",2]}}}}])
    print(sensor_sum)
...
def get_avg(request):
    trip_id ='63d4228e01afc1d6b6813e38'
    sensor_name="GPS"
    sensor_avg=db.vesseltripdata.aggregate([{"$match":{"tripId":trip_id,'sensorName':sensor_name}},{"$addFields": {"arraySize":{"$size":"$dataPoints"}}},{"$group": {"_id":"$sensorName","index0_avg":{"$avg":{"$arrayElemAt":["$dataPoints",0]}},"index1_avg":{"$avg":{"$arrayElemAt":["$dataPoints",1]}},"index2_avg":{"$avg":{"$arrayElemAt":["$dataPoints",2]}}}}])
    print(sensor_avg)
...
def get_sensors(request): 
    trip_id ='63d4228e01afc1d6b6813e38'
    trip_data=db.vesseltripdata.aggregate([{"$match":{"tripId":trip_id}},{"$group": {"_id":"$tripId","sensors":{"$addToSet":"$sensorName"}}}])
    print(trip_data)
